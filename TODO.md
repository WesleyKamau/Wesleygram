...
## ✅ Progress — 2026-07-02 (`fable/web-overhaul` branch → PR)

A full web overhaul on one branch; items marked `[x] ✅ fable/web-overhaul` below.

- **Brand:** pixel-faithful "Wesleygram" wordmark in **Billabong** (the Instagram logo typeface) — HarfBuzz-shaped vector SVG in the headers (zero client font download, `currentColor` theming), plus the font in both OG images. Generator script committed.
- **Share cards:** per-profile branded OG card (photo + wordmark + username) replaces the 302-redirect og:image crawlers couldn't follow; the collage OG is now seeded + statically cached (1h revalidate) instead of regenerating per request.
- **Data/privacy:** metadata stripped to a keep-list (16 internal fields removed incl. `local_path`, `image_hash`, `is_private`, dead CDN URLs) then **blur-up populated: 6,553 LQIP placeholders** — net 5.69 MB → 3.63 MB; stale root dataset copy deleted; regression tests enforce the strip.
- **Security:** `/api/image` + `/api/download` now validate keys against the dataset (bucket no longer enumerable), download filename header sanitized, image `remotePatterns` open-proxy removed, dev metadata editor rewritten JSON-safe + atomic.
- **Perf:** search payload −52 % (1.23 MB, was 2.57 MB); `/search` no longer inlines the dataset into RSC (client fetch, shared cache, CDN-cached API); image fallback now uses R2 originals instead of expired Instagram CDN URLs (fixes broken avatars for unprocessed profiles).
- **Features:** Instagram-style 404, profile stats on mobile (IG placement, hidden when counts missing), `sitemap.xml` (robots.txt line finally true) + canonical profile URLs, SSR skeleton for the home carousel (no more blank first paint).
- **Tests:** vitest + 35 unit tests (search ranking, image URL/fallback logic, homepage split/filter, dataset privacy + allowlist).
- Verified: lint 0 problems, 35/35 tests, clean build, runtime smoke on every route, OG renders visually inspected.
...
- [ ] **🔴 Decide the PII policy for the committed datasets.** `security` `L` · _central legal/ethical decision_ _(progress in fable/web-overhaul: the web copy no longer contains `local_path`, `image_hash`, `is_private`, or Instagram CDN URLs, and the stale root copy is deleted — remaining decision covers bios/counts + the downloader's copies)_
...
- [ ] **🟡 Set up Git LFS / a binary-asset policy.** `infra` `M` _(pressure much lower now: dataset 5.7→3.6MB, notebooks 32MB→141KB)_
...
- [x] **Strip internal/PII fields from `web/src/data/profiles_metadata.json`.** 5.7 MB, git-tracked; 3,297 records leak `local_path` (the dev's Windows path) plus `image_hash`, `status/error`, `r2_*` internals — none used by the site. Strip to display-only fields. — `security` `M` (`web/src/data/profiles_metadata.json`, `web/src/types/index.ts`, `web/src/lib/profiles.ts`) ✅ _fable/web-overhaul: keep-list strip — 16 internal fields removed (incl. `local_path`, `image_hash`, `is_private`, dead `profile_pic_url`); 5.69→1.78MB before blur; unit test locks it in._
...
- [x] **Populate or delete the never-populated LQIP blur-up pipeline.** Fully wired (types `*_blur`, `[id]/page.tsx` blur prop, `placeholder='blur'`) but **0 of 3,298** records have blur strings, so every path falls back to the skeleton. Either run `pnpm generate-blur` (needs R2 creds) or rip out the plumbing. — `incomplete` `M` (`web/scripts/generate-blur.ts`, `web/src/app/[id]/page.tsx`, `web/src/components/ProfileView.tsx`) ✅ _fable/web-overhaul: ran improved `generate-blur` — 6,553 placeholders (3,297 original + 3,256 v2; unused v1 skipped); verified rendering._
- [x] **Reduce the ~2.6 MB profile dataset shipped to every search user.** `getHomeProfiles()` returns all 3,287 profiles incl. `profile_pic_url` (1.53 MB of the 2.57 MB). Served on homepage search focus **and** server-rendered into `/search` props. Drop `profile_pic_url` from `HomeProfile` or move to a paginated server endpoint. — `tech-debt` `M` (`web/src/app/api/profiles/route.ts`, `web/src/lib/profiles.ts`, `web/src/app/search/page.tsx`, `web/src/components/Search.tsx`) ✅ _fable/web-overhaul: dropped dead `profile_pic_url` (fallback now R2 original), /search fetches client-side via shared `useHomeProfiles` — /api/profiles now 1.23MB (-52%), /search RSC payload no longer inlines the dataset._
- [x] **Validate the `key` param in `/api/image` and `/api/download` against known R2 keys.** Both presign/proxy *any* `key` → anyone can enumerate the whole `images-original` bucket. Whitelist against metadata keys. (`w` resize widths are already whitelisted — fine.) — `security` `M` (`web/src/app/api/image/route.ts`, `web/src/app/api/download/route.ts`) ✅ _fable/web-overhaul: `lib/r2-keys.ts` allowlist in both routes (unknown key → 404); download filename header sanitized too._
...
### 🟢 Low priority / polish
- [x] **Add `app/sitemap.ts` or drop the dangling `robots.txt` Sitemap line.** `robots.txt` points to a non-existent `sitemap.xml`. — `docs` `S` (`web/public/robots.txt`) ✅ _fable/web-overhaul: real sitemap (home + /search + every visible profile) + canonical URLs on profile pages._
...
- [x] **Harden or extract the dev-only metadata editor (`/api/profile/metadata`).** Edits the JSON by fragile string surgery (`indexOf("\"<id>\": {")` + `"    },"` heuristic) that can corrupt the file; driven by `window.alert()`. Rewrite with `JSON.parse/stringify` or move out of the deployed app. — `tech-debt` `M` (`web/src/app/api/profile/metadata/route.ts`, `web/src/components/ProfileView.tsx`) ✅ _fable/web-overhaul: rewritten with JSON.parse/stringify + atomic temp-rename write._
- [x] **Narrow `next.config.ts` image `remotePatterns` from `hostname: '**'`** (currently an open image proxy). — `security` `S` ✅ _fable/web-overhaul: remotePatterns removed entirely — only local assets use the optimizer now._
- [x] **Cache the OG collage image** instead of `dynamic = 'force-dynamic'` + `Math.random()` — it regenerates the 1200×630 PNG (font + 16 base64 JPGs) on every request and every page references it. Use `revalidate`/seeded shuffle. — `tech-debt` `M` (`web/src/app/opengraph-image.tsx`, `web/src/app/layout.tsx`) ✅ _fable/web-overhaul: seeded (mulberry32, hourly) + `revalidate = 3600`; now prerendered static._
- [x] **Wire `cache-og-images` into the build (or document the manual refresh).** `public/og-cache` (178 JPGs + manifest, `cachedAt 2025-12-23`) has no prebuild hook → can go stale. — `infra` `S` ✅ _fable/web-overhaul: documented as a manual step in web/README (deliberately no build hook — needs R2 creds)._
...
- [x] **Replace the placeholder lucide Instagram glyph logo** with a real brand mark (`Header.tsx` still has `{/* Placeholder for Instagram-style logo/header */}`). — `enhancement` `S` (`web/src/components/Header.tsx`, `web/src/components/ProfileHeader.tsx`) ✅ _fable/web-overhaul: pixel-faithful "Wesleygram" wordmark in Billabong (the Instagram logo typeface) — HarfBuzz-shaped vector SVG component + OG images; generator script included._
- [x] **Fix `showProfileStats`:** desktop-only (`hidden sm:grid`), gated by an always-true flag, and shows mostly zeros. Populate real counts, show on mobile, or drop the one-flag config module. — `incomplete` `S` (`web/src/lib/config.ts`, `web/src/components/ProfileView.tsx`) ✅ _fable/web-overhaul: stats now visible on mobile in the Instagram position (under avatar row); hidden only when all counts are 0 (missing data)._
- [x] **Per-profile OG/Twitter image uses a `/api/image` 302 redirect** — some crawlers don't follow redirects for OG images, so share cards may render blank. Emit the presigned URL directly (already resolved server-side) or add a per-profile `opengraph-image` route. — `bug` `M` (`web/src/app/[id]/page.tsx`) ✅ _fable/web-overhaul: branded per-profile share card at `[id]/opengraph-image.tsx` (real bytes: photo + wordmark + username)._
- [x] **Move carousel shuffling server-side (seeded).** `HomePreview*` shuffle in `useEffect` via `Math.random`, returning null until the effect runs → hydration flash + it's the root cause of the set-state-in-effect errors. — `tech-debt` `M` (`web/src/components/HomePreview.tsx`, `web/src/components/HomePreviewDesktop.tsx`, `web/src/lib/homepage.ts`) ✅ _fable/web-overhaul: solved differently — shuffle stays client-side (per-visitor variety) but SSR now paints a matching skeleton, so no blank flash; lint errors were fixed earlier._
...
- [x] **Add a unit-test setup (e.g. vitest)** for the core pure functions — `searchRankProfiles`, `selectProcessedKey`/`getImageUrl`, `getHomeProfiles`/`getCarouselProfiles`, `filterHomepageProfiles`/`splitIntoRows`. Zero tests today. — `tech-debt` `M` (`web/src/lib/`) ✅ _fable/web-overhaul: vitest + 35 tests across search/images/homepage/profiles incl. dataset-privacy regression tests._
...
- [ ] **Scrub `local_path` + reconsider PII across the remaining committed metadata copies** _(fable/web-overhaul scrubbed the web copy + deleted the root copy; `instagram_downloader/data/` copies remain)_ (`web/src/data/`, root, `instagram_downloader/data/`). Every profile carries the dev's absolute Windows path; ~645 private accounts ship bios/counts. — `security` `M`
